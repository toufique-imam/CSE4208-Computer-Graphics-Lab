...
void bezier(point *control_points_array,
...
void display()
{
    ...
    for(int i=0;i<4;i++){
        cout<<control_points_array[i].x<<" "<<control_points_array[i].y<<"\n";
    }
    //exit(0);
    ...
    glColor3f(0, 1, 0); //Indian flag: Saffron color code
    for(int i=0; i<50; i++)
    {
    ...
    glColor3f(0, 1, 0); //Indian flag: white color code
    for(int i=0; i<50; i++)
    {
        glTranslatef(0, -0.8, 0);
        bezier(control_points_array, number_of_control_points, number_of_bezier_points);
